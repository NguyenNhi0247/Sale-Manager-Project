...
def insert_order_by_user_id(uid):
    now = datetime.now()
    ...
    insert_book_orders(uid, list_book_cart)
   

def insert_book_orders(uid, list_book_cart):
    order = get_order_by_user_id(uid)
    for book_cart in list_book_cart:
        new_bookorder = BookOrders(
            book_id = book_cart.book_id,
            order_id = order.id,
            price = book_cart.price,
            quantity = book_cart.quantity,
            updated_at = now,
        )
        save_changes(new_bookorder)
